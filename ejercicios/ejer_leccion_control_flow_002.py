...
most_win_director = []
most_aux_director_win = {}
# Add your code here
for i, age in enumerate(winners):
    for winner in winners[age]:
        if winner in most_aux_director_win:
            most_aux_director_win[winner] = most_aux_director_win[winner] + 1
        else:
            most_aux_director_win[winner] = 1
print(most_aux_director_win)
maxprime = 0
for i,director in enumerate(most_aux_director_win) :
    if most_aux_director_win[director] > maxprime :
        maxprime = most_aux_director_win[director];
        most_win_director.clear()
        most_win_director.append(director)
    else :
        if most_aux_director_win[director] == maxprime :
            most_win_director.append(most_aux_director_win[director])
